...
| SDKセッションID | Claude SDKの会話コンテキスト | セッション削除まで |

**セッション再開機能:**

ブラウザを更新したり、ページを閉じて再度開いた場合でも、AIとの会話コンテキストは維持されます。これにより、長時間の作業や中断後の再開がスムーズに行えます。

| 状況 | 動作 |
|------|------|
| ブラウザ更新 | セッションを選択し直すと前回の会話から続行可能 |
| ページを閉じる→再度開く | セッション一覧から選択して作業再開 |
| 複数タブ/デバイス | 同一セッションに同時接続可能 |
