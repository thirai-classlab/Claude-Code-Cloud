...
**重要：構造化された情報は箇条書きではなくテーブル形式で記載すること。**

テーブル形式は視認性が高く、情報の比較・参照が容易になります。

| 使用場面 | 理由 |
|----------|------|
| 項目と説明の対応 | 一覧性が高い（API一覧、パラメータ等） |
| 複数属性を持つリスト | 比較しやすい（テーブル定義、カラム仕様等） |
| 選択肢・オプション | 選択条件が明確（コマンド一覧、設定項目等） |
| ステータス・状態 | 状態遷移が明確（進捗状況、機能一覧等） |

箇条書きは以下の場合のみ使用：手順・ステップ（番号付き）、単純な列挙、注意事項・補足、階層構造（ディレクトリ等）
